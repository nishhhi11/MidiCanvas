...
export default function LibraryPage() {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-main)] flex flex-col relative">
            ...
            >
                <div className="max-w-[1400px] mx-auto space-y-8">
                    ...
                    <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--primary)]/10 pb-8">
                        <div>
                            ...
                            <p className="text-[var(--text-muted)] text-base">Your saved projects live here — forever in your browser.</p>
                        </div>
                        <button 
                            onClick={triggerUpload}
                            className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[var(--primary)] text-[var(--bg-primary)] text-sm hover:scale-105 font-bold uppercase tracking-wider ivory-glow transition-all"
                        >
                            <Upload size={18} />
                            Upload New
                        </button>
                        ...
                        <div className="flex items-center gap-4 glass-panel p-2 rounded-xl">
                            <div className="relative flex-1">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-12 pr-4 py-2.5 bg-transparent rounded-lg focus:outline-none focus:bg-[var(--primary)]/5 transition-all text-[var(--text-main)] placeholder-[var(--text-muted)] border border-transparent focus:border-[var(--primary)]/20"
                                />
                            </div>
                            <div className="w-px h-6 bg-[var(--primary)]/10" />
                            <div className="flex gap-4 pr-2">
                                <select 
                                    value={sortBy}
                                    onChange={(e) => setSortBy(e.target.value)}
                                    className="px-4 py-2.5 bg-transparent focus:outline-none text-[var(--text-muted)] hover:text-[var(--text-main)] cursor-pointer appearance-none min-w-[140px] border border-transparent focus:border-[var(--primary)]/20 rounded-lg transition-all"
                                ...
                                >
                                    <option value="date" className="bg-[var(--bg-secondary)]">Sort by: Date</option>
                                    <option value="name" className="bg-[var(--bg-secondary)]">Sort by: Name</option>
                                    <option value="size" className="bg-[var(--bg-secondary)]">Sort by: Size</option>
                                    <option value="tracks" className="bg-[var(--bg-secondary)]">Sort by: Tracks</option>
                                </select>
                            </div>
                        </div>
                    )}
            ...
            <div className="fixed bottom-0 left-0 w-full glass-panel border-b-0 border-x-0 z-50">
                <div className="max-w-[1400px] mx-auto px-6 h-8 flex items-center justify-between text-[10px] uppercase font-bold tracking-widest text-[var(--text-muted)]">
